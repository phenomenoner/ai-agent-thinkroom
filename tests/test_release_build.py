...
def test_release_builder_pins_frontend_backend_and_epoch(tmp_path, monkeypatch):
    ...
    def fake_run(command, *, cwd, env, check):
        ...
        (output / "thinkroom-0.2.3-py3-none-any.whl").write_bytes(b"wheel")
        (output / "thinkroom-0.2.3.tar.gz").write_bytes(b"sdist")
    ...
    assert sorted(path.name for path in output.iterdir()) == [
        "thinkroom-0.2.3-py3-none-any.whl",
        "thinkroom-0.2.3.tar.gz",
    ]
...
def test_release_builder_rejects_malformed_archives(tmp_path):
    ...
    output = tmp_path / "dist"
    output.mkdir()
    (output / "thinkroom-0.2.3-py3-none-any.whl").write_bytes(b"not a wheel")
    (output / "thinkroom-0.2.3.tar.gz").write_bytes(b"not an sdist")
