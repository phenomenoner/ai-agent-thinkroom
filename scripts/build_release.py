...
def _validate_wheel(path: Path) -> None:
    try:
        with zipfile.ZipFile(path) as archive:
            ...
            if "Name: thinkroom\n" not in metadata_text or "Version: 0.2.3\n" not in metadata_text:
                raise ValueError
    ...
    try:
        with tarfile.open(path, "r:gz") as archive:
            ...
            if any(
                ...
                raise ValueError
            required = {
                "thinkroom-0.2.3/pyproject.toml",
                "thinkroom-0.2.3/PKG-INFO",
            }
            if not required.issubset(names):
                raise ValueError
            pkg = archive.extractfile("thinkroom-0.2.3/PKG-INFO")
            if pkg is None:
                raise ValueError
            metadata = pkg.read().decode("utf-8")
            if "Name: thinkroom\n" not in metadata or "Version: 0.2.3\n" not in metadata:
                raise ValueError
