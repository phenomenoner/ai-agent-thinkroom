...
def _validate_wheel(path: Path) -> None:
    try:
        with zipfile.ZipFile(path) as archive:
            ...
            if "Name: thinkroom\n" not in metadata_text or "Version: 0.2.0\n" not in metadata_text:
                raise ValueError
    ...
    try:
        with tarfile.open(path, "r:gz") as archive:
            ...
            if any(
                ...
                raise ValueError
            required = {
                "thinkroom-0.2.0/pyproject.toml",
                "thinkroom-0.2.0/PKG-INFO",
            }
            if not required.issubset(names):
                raise ValueError
            pkg = archive.extractfile("thinkroom-0.2.0/PKG-INFO")
            if pkg is None:
                raise ValueError
