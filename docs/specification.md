...
`prime_agent_failover` invokes one explicitly configured Prime Agent primary and one explicitly configured fallback under the contract in [Provider resilience and progress](provider-resilience-v0.2.6.md). A fast transient or rate limit may retry the same route once; a timeout skips same-route retry. Availability or an unclassified error may then use the fallback. A primary `OUTPUT_LIMIT_RAW_TRANSPORT` at the absolute wire ceiling never retries the primary: it contributes two points to the attempt-local primary circuit and may consume the one configured fallback if the shared three-call physical budget and deadline still permit it. `OUTPUT_LIMIT_ACCOUNTED_TRANSPORT`, the same absolute-wire error from the fallback, and every other output-limit subtype are terminal; no invalid or partial output is salvaged. Cancellation, fencing, and deadline exhaustion are also terminal. Each `(attempt, phase, branch)` is limited to three physical calls across retry, fallback, and route-preserving schema repair. Persisted call evidence reconstructs an attempt-local primary circuit after restart and prevents new primary calls after one timeout, one primary absolute raw-transport overflow, or two fast transient failures; rate limits do not contribute. No credential, raw provider error, prompt, response, or telemetry value is persisted.
...
- Prime RPC input, argv, each LF-delimited event, absolute raw transport bytes, amplification-aware
  accounted transport bytes, semantic event count, discarded `message_update` telemetry event count,
  retained result/control bytes, terminal assistant text, timeout, and cleanup SHALL be bounded.
  The per-line ceiling SHALL remain 64,000,000 bytes, the accounted total SHALL remain 64,000,000
  bytes, and the independent absolute raw total SHALL be 512,000,000 bytes per physical call.
  Every event SHALL consume at least 128 accounted bytes, and an accounting projection nested beyond
  64 container levels SHALL fail as `MALFORMED_PROVIDER_OUTPUT`. A non-`message_update` event consumes its
  complete raw line length. A `message_update` consumes the canonical UTF-8 JSON byte length of a
  projection that removes exactly the top-level `message` value and exactly the nested
  `assistantMessageEvent.partial` value while retaining every other known or unknown field; the
  projection is measured and discarded and SHALL never provide semantic result, lifecycle, custody,
  cleanup, or ordering evidence. Renamed, relocated, or additional fields receive no discount.
  Stderr SHALL be drained without
  retention and SHALL NOT participate in result validity; its task SHALL be cancelled after a valid
  JSONL result. A `message_update` remains untrusted and consumes the telemetry-event, accounted-byte,
  and absolute-raw ceilings, but SHALL NOT consume the lower semantic-event ceiling used for
  ...
  SHALL preserve that status. `OUTPUT_LIMIT_ACCOUNTED_TRANSPORT` SHALL be terminal without retry or
  fallback. A primary `OUTPUT_LIMIT_RAW_TRANSPORT` failure at the absolute raw ceiling MAY consume one
  remaining physical-call slot on the configured fallback without retrying primary and SHALL add two
  points to the attempt-local primary circuit; all other output-limit sites and a fallback raw-
  transport failure remain terminal. Terminal job/API/MCP errors SHALL continue to expose only the
  stable public code and safe message. The production process wrapper SHALL retain same-group descendant custody until
...
JSON payloads are versioned. An empty database receives the current canonical schema. The exact canonical v0.2.4 provider-call schema may migrate to v0.2.5 by appending `route_role`, `effective_timeout_seconds`, `error_code`, `transport_bytes`, `transport_events`, `transport_max_event_bytes`, `transport_message_updates`, `transport_snapshot_bytes`, `transport_partial_bytes`, and `transport_delta_bytes`, then re-attesting the complete v0.2.5 shape. The exact canonical v0.2.5 schema may migrate to v0.2.6 by appending `transport_accounted_bytes`, then re-attesting the complete v0.2.6 shape before readiness. A multi-hop v0.2.4-to-v0.2.6 migration SHALL execute all DDL and final attestation within one explicit SQLite transaction; any exception SHALL roll back the complete chain. Transport fields are nullable non-negative integer counters. They never contain prompt, response, event, provider-error, or credential text and remain null for a backend without measurement support. A schema-invalid Prime result SHALL settle its physical call with the measurements already returned by that invocation rather than discarding them during phase-model validation. Any other existing noncanonical or prerelease schema is rejected before live DDL or other writes and readiness remains false. A future schema migration requires another explicit versioned contract and independent release evidence before readiness may succeed.
...
Given a valid Prime RPC lifecycle whose cumulative `message_update` snapshots push raw wire volume above 64,000,000 bytes but remain below the 512,000,000-byte absolute ceiling, the phase succeeds when its canonical accounted projection remains within 64,000,000 bytes. A replay in which the final answer exists only inside `message_update` still fails closed. Renamed or additional telemetry fields are charged, every line consumes at least 128 accounted bytes, and non-telemetry events consume their complete raw line lengths. Each Prime invocation records total raw bytes, accounted bytes, total events, largest-event bytes, telemetry-event count, cumulative serialized `message_update.message` bytes, cumulative serialized `message_update.assistantMessageEvent.partial` bytes, and cumulative serialized `assistantMessageEvent.delta` bytes without retaining their values. A legacy top-level delta is counted only when the nested assistant event is absent. The same bounded numeric snapshot is attached to a typed Prime backend failure, crosses the process boundary, and settles on the physical provider-call row; a later phase-model validation failure retains the invocation measurements on its `MALFORMED_PROVIDER_OUTPUT` row. Exceeding the semantic-event, telemetry-event, accounted-byte, absolute-raw-byte, per-event, control-byte, or terminal-text ceiling fails with public code `OUTPUT_LIMIT_EXCEEDED` and preserves the corresponding safe audit status through process isolation and physical provider-call settlement. An accounted-transport limit is terminal. A primary absolute raw-transport limit never retries primary, opens the attempt-local primary circuit, and may use the configured fallback once within the existing three-call budget; all other output-limit sites and a fallback raw-transport limit remain terminal. Measurement never bypasses a ceiling and incomplete or schema-invalid JSON is never salvaged. An initial fork output-limit failure completes with deterministic perspectives only after this bounded route policy cannot produce a valid provider result; the same unresolved failure in framing remains terminal.

### AC-012 - Phase budget and admission evidence

A physical failover route SHALL enforce its configured timeout around the complete adapter invocation, including process startup. Cancellation SHALL drain before another route starts; a late return after timeout SHALL not validate as success. A retry SHALL be skipped when its delay and configured primary/fallback timeouts cannot fit together.

The hard job deadline SHALL remain fixed, without subtracting fixed finalization reserves from early phases. Soft admission expiry SHALL stop new framing, fork, and rollout admissions but SHALL NOT by itself discard finalization of available successful branches. Critique and synthesis SHALL attempt to use the remaining hard window subject to configured route admission timeouts. Missing branch admissions SHALL keep the result partial and identify skipped branches even when synthesis is present. The legacy partial response schema SHALL remain unchanged. A separate `GET /api/v1/research/{job_id}/diagnostics` query SHALL expose current-attempt admission artifacts recording no provider start, semaphore wait duration, and phase cutoffs; old jobs without artifacts SHALL return an empty list. Physical call counts SHALL exclude rejected admissions. A deadline too short to admit even the initial non-failover provider SHALL retain `DEADLINE_INSUFFICIENT`.
