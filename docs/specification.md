...
`prime_agent_failover` invokes one explicitly configured Prime Agent primary and one explicitly configured fallback under the contract in [Provider resilience and progress](provider-resilience-v0.2.5.md). A fast transient or rate limit may retry the same route once; a timeout skips same-route retry. Availability or an unclassified error may then use the fallback. A primary `OUTPUT_LIMIT_RAW_TRANSPORT` never retries the primary: it contributes two points to the attempt-local primary circuit and may consume the one configured fallback if the shared three-call physical budget and deadline still permit it. The same error from the fallback and every other output-limit subtype are terminal; no invalid or partial output is salvaged. Cancellation, fencing, and deadline exhaustion are also terminal. Each `(attempt, phase, branch)` is limited to three physical calls across retry, fallback, and route-preserving schema repair. Persisted call evidence reconstructs an attempt-local primary circuit after restart and prevents new primary calls after one timeout, one primary raw-transport overflow, or two fast transient failures; rate limits do not contribute. No credential, raw provider error, prompt, or response is persisted.
...
- Prime RPC input, argv, each LF-delimited event, raw transport bytes, semantic event count,
  ...
  SHALL preserve that status. A primary `OUTPUT_LIMIT_RAW_TRANSPORT` failure MAY consume one
